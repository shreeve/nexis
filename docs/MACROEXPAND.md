...
| `case` | Keys are constants, never evaluated: a symbol key is that symbol, a vector or map key that literal, a list `(k1 k2)` groups alternatives; they are compared with `=`. With fewer than three constants, `(let* [g expr] (if (= g 'k1) v1 ...))`; with three or more, one hashed lookup finds the clause, `(let* [g expr i (get '{k1 0 k2 1 ...} g -1)] (if (== i 0) v1 (if (== i 1) v2 ...)))`, each test one inlined compare (`g` is bound only when there is no default, for the throw). The map's lookup is `=`'s equality and hash, and its entries go in last clause first, so of two constants that are `=` but spelled differently (`[1]`, a grouped `(1)`) the first clause wins, as with the chain. A constant given twice, alone or in a group, is `MalformedMacroCall` "case: duplicate test constant" at the second, as in Clojure; constants of different kinds (`1`, `1.0`, `\1`) are distinct. With no trailing default, no match throws `{:error :no-matching-clause :message "No matching clause: <expr>" :value expr}`. |
