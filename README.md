...
| Nextomic query | ✅ shipped | `d/q` and `d/explain` over the Datalog pipeline (§5): patterns, every `:in` form, predicates and function bindings that call any Lisp function through the namespace registry, aggregates, every find spec, `not`/`or` and their `-join` forms, recursive rules, time views; parsed queries cached per VM; `test/nextomic/query.nx`. `pull` and `with` are next. |
